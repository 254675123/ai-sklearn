...
def restore_image(cluster, cb, shape):
    row, col, dummy = shape
    image = np.empty((row, col, 3))
    index = 0
    for r in range(row):
        for c in range(col):
            image[r, c] = cluster[cb[index]]
            index += 1
    return image
...
if __name__ == '__main__':
    ...
    num_vq = 256                            # 图片最后保留的维度，或者叫不重复像素值的个数
    im = Image.open('./../data/lena.jpg')     # flower2.png(200)/lena.png(50)
    image = np.array(im).astype(np.float) / 255
    image = image[:, :, :3]                 # 去掉透明度alpha
